...
public class SysuserHandler extends BaseHandler {
    ...
    public AxiosResult createValicode(@PathVariable("uphone") String uphone){
        ...
        String code = sysuserService.sendValicode(uphone, s);
        return suc(s);
    }

    @PostMapping("loginByPhone/{uphone}/{code}")
    public ResponseEntity loginByPhone(@PathVariable("uphone")String uphone, @PathVariable("code") String code){
        // 比对验证码
        ...
        sysuser.setUpwd("");
        // 加入redis缓存数据库中
        rs.hset(Keys.PHONE_HASH,uphone,JSON.toJSONString(sysuser));
        String token = JJWTUtil.generate(uphone);

//        CurrentLoginUser.setLoginUser(sysuser);
        HttpHeaders headers = new HttpHeaders();
        headers.add("token",token);
        return new ResponseEntity(suc(sysuser),headers,HttpStatus.OK) ;
    }

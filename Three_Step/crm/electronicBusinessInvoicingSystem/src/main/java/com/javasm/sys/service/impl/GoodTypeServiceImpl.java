...
public class GoodTypeServiceImpl extends BaseServiceImpl<GoodTypeDao,GoodType> implements GoodTypeService {
    ...
    @Override
    public List<GoodType> selectByType(String type) {
        List<GoodType> list=dao.selectByType(type);
        System.out.println(1);
        return list;
    }

    @Override
    public List<GoodType> selectChild(String id) {
        List<GoodType> typeList=new ArrayList<>();
        GoodType goodType = dao.selectById(id);
        childList(typeList,goodType);
        return typeList;
    }
    private List<GoodType> childList(List<GoodType> typeList,GoodType type){

        List<GoodType> list = dao.selectChild(type.getTid().toString());
        for (GoodType goodType : list) {
            typeList.add(goodType);
            childList(typeList,goodType);
        }
        return typeList;
    }
//    private  List<GoodType> getChildList(List<GoodType> list){
//        for (GoodType goodType : list) {
//
//            for (GoodType type : childList(goodType)) {
//                list.add(type);
//                List<GoodType> typeList = dao.selectChild(type.getTid().toString());
//                getChildList(typeList);
//            }
//        }
//        return list;
//    }
    ...
    private List<GoodType> tree(List<GoodType> list,Set<Integer> pids){
        ...
        for (GoodType goodType : list) {
            ...
            if (pids.contains(pid)) {
                ...
                if (childList.size()>0) {
                    goodType.setChildren(childList);
                }
                result.add(goodType);
            }
        }
    ...
    private List<GoodType> childs(List<GoodType> goodTypes,Integer pid){
        List<GoodType> children=new ArrayList<>();

...
public class LogHandler extends BaseHandler {
    ...
    @PostMapping("modules")
    public AxiosResult getModule() {
        //startPage();
        List<Log> list = this.logService.selectList(null);
        System.out.println(list.size());
        list = logService.filterRepeatModule(list);
        return toTableDatas(list);
    }

    @PostMapping("OperatorContent")
    public AxiosResult getOperatorcontent() {
        //startPage();
        List<Log> list = this.logService.selectList(null);
        System.out.println(list.size());
        list = logService.filterOperatorcontent(list);
        return toTableDatas(list);
    }

    @PostMapping("deptNames")
    public AxiosResult getdeptNames() {
        //startPage();
        List<Log> list = this.logService.selectList(null);
        System.out.println(list.size());
        list = logService.filterDeptNames(list);
        return toTableDatas(list);
    }
    ...
    @GetMapping("keyWords")
    public AxiosResult selectKeyWords(String keyWord) {
        System.out.println("word=="+keyWord);
        startPage();
        List<Log> logs = logService.selectByKeyWords(keyWord);
        return toTableDatas(logs);

    }

    @GetMapping("ManyParam")
    public AxiosResult selectManyTerm(Log log) {
        startPage();
        List<Log> logs = logService.selectManyTerm(log);
        return toTableDatas(logs);

...
public class Utils {
    ...
    private static UserDao userDao = Constant.userDao;
    // 标记当前已登录对象所对应的数组索引
...
//        int loggedIdex = -1;
        if (!Utils.isNull(arr)) {
            for (int i = 0; i < arr.length; i++) {
                if (name.equals(arr[i].getName()) && password.equals(arr[i].getPassWord())) {
                    isFlag = true;
    //                loggedIdex = i;
                    break;
                }
    ...
    public static int findIndex(String userName, User[] userArr) {
        for(int i = 0; i<userArr.length;i++){
            if(userName.equals(userArr[i].getName())){
                return i;
            }
        }
        return -1;
    ...
    // 根据公司名字找到公司的索引
    public static int findIndex(String cmpName) {
        for (int i = 0; i < MainFuc.companyMgr.getCompaniesArr().length; i++) {
            if (cmpName.equals(MainFuc.companyMgr.getCompaniesArr()[i].getCompanyName())) {
                return i;
            }
        }
        return -1;
